...
double my_random()                //Return a random decimal b/n 0 and 1
{
...
public:
    ...
    //Sigmoid, hyperbolic tangent, sine, cosine
	double f(double x) { 
        if (act_func == 1) 
            return 1.0 / (1.0 + exp(-x)); 
        else if (act_func == 2) return tanh(x); 
        else if (act_func == 3) return sin(x); 
        else if (act_func == 4) return cos(x); 
        else if (act_func == 5) return x; 
        else {
            cerr << "Invalide activation function!" << endl;
            exit(0);
        }
    } 

	double f_derv(double x) { 
        if (act_func == 1) return f(x)*(1 - f(x)); 
        else if (act_func == 2) return 1 - pow(tanh(x), 2); 
        else if (act_func == 3) return cos(x); 
        else if (act_func == 4) return -sin(x); 
        else if (act_func == 5) return 1; 
        else {
...
{
	//First layer
	for (int i = 0; i<(int)Neural_Net[0].size() - 1; ++i)//Exclude the bias
	{
	...
	for (int i = 1; i<(int)Neural_Net.size(); ++i)
	{
		int end = Neural_Net[i].size() - 1; //Excludes the bias
		if (i == (int)Neural_Net.size() - 1)//the output layer doesn't have a bias neuron.
			end += 1;
		...
		{
			double sum = 0.0;
			for (int k = 0; k<(int)Neural_Net[i - 1].size(); ++k) //Calculate the sum of the w*o of the previous layer
...
void Net::back_prop(const double &target)
{
	...
	{
		for (int j = 0; j<(int)Neural_Net[i].size(); ++j)//Every Neuron in layer i even the bias neuron. The bias's weights still change
		{
			for (int k = 0; k<(int)Neural_Net[i][j].weights.size(); ++k)//Every Weight
			{
				if (i == (int)Neural_Net.size() - 2) //Gradient calculation for the second last layer
				{
					if(!scale)
				...
				{
					double sum_0 = 0.0;
					for (int h = 0; h <(int) Neural_Net[i + 1][k].weights.size(); ++h)
					{
